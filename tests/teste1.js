...
function initialize() {
	...
	for ( var c = 0; c < numObjs; c++) {
		array[c] = {
			...
			color : 'rgb(' + (Math.floor(Math.random() * 255) + 100) + ','
					+ (Math.floor(Math.random() * 255) + 100) + ','
					+ Math.floor(Math.random() * 0) + ')',
			bounce : Math.abs(Math.random() - 0.10),
			rot : 0.0,
			angvel : clamp(Math.random(), 0, 0)
		};
	}
}
...
function draw(ctx) {
	...
	for ( var c = 0; c < numObjs; c++) {
		ctx.save();
		ctx.translate(array[c].x + array[c].size / 2, array[c].y
				+ array[c].size / 2);
		ctx.rotate(array[c].rot);
		ctx.fillStyle = array[c].color;

		ctx.fillRect(-array[c].size / 2, -array[c].size / 2, array[c].size,
				array[c].size);
		ctx.restore();
	}
}
